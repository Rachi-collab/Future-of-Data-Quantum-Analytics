...
st.caption(
    "Preprocess and profile datasets before feeding them into classical or quantum models."
)
...
# ── Generate data ─────────────────────────────────────────────────────────────
X, y, feat_names = generate_dataset(
    dataset, n_samples=n_samples, noise=noise, random_state=seed
)
...
st.markdown(
    '<div class="section">📋 Descriptive Statistics</div>', unsafe_allow_html=True
)
...
with col_a:
    st.markdown(
        '<div class="section">🎯 Class Distribution (Scatter)</div>',
        unsafe_allow_html=True,
    )
    ...
    for cls, color in palette.items():
        mask = y == cls
        ax.scatter(
            X[mask, 0], X[mask, 1], c=color, alpha=0.6, s=18, label=f"Class {cls}"
        )
...
with col_b:
    st.markdown(
        '<div class="section">📈 Feature Distributions</div>', unsafe_allow_html=True
    )
...
with col_c:
    st.markdown(
        '<div class="section">🔗 Correlation Matrix</div>', unsafe_allow_html=True
    )
    fig, ax = plt.subplots(figsize=(5, 4))
...
with col_d:
    st.markdown(
        '<div class="section">📦 Outlier Detection (Box Plots)</div>',
        unsafe_allow_html=True,
    )
    fig, ax = plt.subplots(figsize=(5, 4))
    data_to_plot = [df[fn].values for fn in feat_names]
    bp = ax.boxplot(
        data_to_plot, patch_artist=True, medianprops=dict(color="#00d4ff", linewidth=2)
    )
...
st.success(
    f"✅ Dataset '{dataset}' ready · {len(X_tr)} train / {len(X_te)} test samples"
)
